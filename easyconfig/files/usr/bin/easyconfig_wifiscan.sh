...
iw dev $IFACE1 scan 2>/dev/null | awk '{gsub("(on","");if($1 == "BSS") {MAC = $2; wifi_enc[MAC] = ""; wifi_chan[MAC] = "?"; wifi_mode0[MAC] = ""; wifi_mode1[MAC] = ""; wifi_mode2[MAC] = ""}
if($1 == "SSID:") {t="";for(i=2;i<=NF-1;++i){t=t$i" "};if(NF<2){t=""}else{t=t$NF}; wifi_ssid[MAC]=t}
if($1 == "signal:") {wifi_sig[MAC] = $2}
if($1 == "freq:") {wifi_freq[MAC] = $2; if($2<5000){wifi_mode0[MAC] = "2.4GHz"; wifi_mode1[MAC] = "g"}else{wifi_mode0[MAC] = "5GHz"; wifi_mode1[MAC] = "a"}}
...
for (w in wifi_enc) {
    printf "{\"mac\":\"%s\",\"ssid\":\"%s\",\"freq\":\"%s\",\"signal\":\"%s\",\"channel\":\"%s\",\"encryption\":\"%s\",\"mode1\":\"%s\",\"mode2\":\"%s\",\"mode3\":\"%s\"},\n", w, wifi_ssid[w], wifi_freq[w], wifi_sig[w], wifi_chan[w], wifi_enc[w], wifi_mode0[w], wifi_mode1[w], wifi_mode2[w]
}
}'
done
echo '{"mac":"","ssid":"","freq":"","signal":"","channel":"","encryption":"","mode1":"","mode2":"","mode3":""}'
echo ']'
exit 0
