...
var garCronWIFI = "/usr/lib/gargoyle/usbrelay.sh";
var new_cron_tabs = [];
...
function InitSummaryText() {
	...
	textSpan.innerHTML="<strong>"+Usch.Smmy+":</strong><br />\n";
}
...
function generateCronTabStr(min, hour, day, extra) {
	...
	if (min == 0) {
		a_cron_string="0 " + hour + " * * " + day_string + " " + garCronWIFI + " " + "off " + usbrelay_port + extra;
	} else if (min == 60) {
		a_cron_string="0 " + hour + " * * " + day_string + " " + garCronWIFI + " " + "on " + usbrelay_port + extra;
	} else {
		a_cron_string="" + Math.abs(min) + " " + hour + " * * " + day_string + " " + garCronWIFI + " " + (min > 0 ? "off " : "on ") + " " + usbrelay_port + extra;
	}
...
function CronWarning() {
	...
	if ( (Wi_Fi == 1 && thishour.value == 0) || (Wi_Fi == -1 && thishour.value == 60) ) {
		AddSummaryText("<br/>\n<strong>"+Usch.Warn+":</strong><br/>\n");
		AddSummaryText(Usch.NextEv+"<br/>\n");
...
function UpdateSummary() {  //summary is dynamically generated from parsed crontab text
	...
	AddSummaryText(Usch.SelTM+": ");
	if (timerMode == 0) { AddSummaryText(Usch.SumDis+"<br />\n"); }
	if (timerMode == 1) { AddSummaryText(Usch.SumDly+"<br />\n"); }
	if (timerMode == 3) { AddSummaryText(Usch.SumSwS+"<br />\n"); }
	if (timerMode == 7) { AddSummaryText(Usch.SumWky+"<br />\n"); }
	...
	for(var ii = 0; ii < new_cron_tabs.length; ii++) {
		...
		if (dayCronText == "*") { day_string = Usch.STDly; }
		if (dayCronText == "0") { day_string = Usch.STSunday; }
		if (dayCronText == "1") { day_string = Usch.STMonday; }
		if (dayCronText == "2") { day_string = Usch.STTuesday; }
		if (dayCronText == "3") { day_string = Usch.STWednesday; }
		if (dayCronText == "4") { day_string = Usch.STThursday; }
		if (dayCronText == "5") { day_string = Usch.STFriday; }
		if (dayCronText == "6") { day_string = Usch.STSaturday; }
		if (dayCronText == "1-5") { day_string = Usch.STMonFri; }
		
		AddSummaryText(Usch.SumGo+" " + (wifiCronCMD.search("on") >= 0 ? Usch.SumUp : Usch.SumDn) + " - " +  day_string + " "+Usch.SumAt+" " + (hourCronText < 10 ? '0' + hourCronText : hourCronText) + ":" + minuteStr + "<br />\n");
	}
...
function GenerateCellData(table, row, column, col_label) {
	...
	table.rows[row+1].cells[column].value=60; // Cezary - for usb relay
...
function SeatCronData(cron_minute, cron_hour, cron_day, cron_cmd) {
	...
	if (ecron_minute > 0 && ecron_minute < 60) {
		...
		if (cron_cmd.search("on")>=0) { ecell.value = ecell.value*-1; }
		CellGradient(ecell);
		ecell.innerHTML = ecell.value;
	} else if (cron_cmd.search("on") >= 0) {
		SetCellContents(ecell, "60", 1000, hour_green);
	}  else if (cron_cmd.search("off") >= 0) {
...
function SetWifiStatus(shell_iwconfig) {
	var iwconfig = (shell_iwconfig == null ? usbrelay_status : shell_iwconfig);
	if (iwconfig.length > 0 && iwconfig[0] == "on") {
		Wi_Fi = 1;
	} else {
		Wi_Fi = -1;
	}
	if (found_wifi_cron_tabs.length > 0 && timerMode > 0) { //this causes a display hiccup when there are crontabs on the router, hit reset button (scheduled) disappears, but there is still a schedule
		...
		if (this_cron_hour == 60 || this_cron_hour == 0) {
			if (Wi_Fi > 0) {
				setChildText("wlan_status", (this_cron_hour == 60 ? Usch.actv+" ("+Usch.schd+")" : Usch.actv+" ("+Usch.nscd+")") );
			} else {
				setChildText("wlan_status", (this_cron_hour == 60 ? UI.disabled+" ("+Usch.nscd+")" : UI.disabled+" ("+Usch.schd+")") );
			}
		} else { //minutes on the hour
			if (this_cron_hour > 0) {
				if (Wi_Fi > 0) {
					setChildText("wlan_status", (this_cron_hour > current_time[2] ? Usch.actv+" ("+Usch.schd+")" : Usch.actv+" ("+Usch.nscd+")") );
				} else {
					setChildText("wlan_status", (this_cron_hour > current_time[2] ? UI.disabled+" ("+Usch.nscd+")" : UI.disabled+" ("+Usch.schd+")") );
				}
			} else {
				if (Wi_Fi > 0) {
					setChildText("wlan_status", (current_time[2] > Math.abs(this_cron_hour) ? Usch.actv+" ("+Usch.nscd+")" : Usch.actv+" ("+Usch.schd+")") );
				} else {
					setChildText("wlan_status", (current_time[2] > Math.abs(this_cron_hour) ? UI.disabled+" ("+Usch.nscd+")" : UI.disabled+" ("+Usch.schd+")") );
				}
			}
		}
	} else {
		setChildText("wlan_status", (Wi_Fi > 0 ? Usch.actv : UI.disabled), (Wi_Fi > 0 ? "green" : "red") );
	}
}
...
function LoadCrontabs() {
	...
	setChildText('usbrelay_port', ''+usbrelay_port);
	var name = uciOriginal.get("usbrelay", "desc", '' + usbrelay_port);
	if (name == "") { name="..."; }
	document.getElementById("usbrelay_name").value=name;
	...
	dailyPeriod = [ Usch.Dly ];
	week511Period = Usch.WDayA;
...
function saveChanges() { 	//follow reboot.sh somewhat
	...
	var name = document.getElementById("usbrelay_name").value;
	var name_orig = uciOriginal.get("usbrelay", "desc", "" + usbrelay_port);
	if (name_orig == "") { name_orig = "..."; }
	if (name_orig != name) {
		commands.push("uci set usbrelay.desc." + usbrelay_port + "=\"" + name + "\"");
		commands.push("uci commit usbrelay");
		uciOriginal.set("usbrelay", "desc", "" + usbrelay_port, name);
	}

	commands.push("mkdir -p /etc/crontabs"); //should fail gracefully
	commands.push("touch /etc/crontabs/root"); //no harm, no foul
	commands.push("cat /etc/crontabs/root | grep -v -e '" + garCronWIFI + ".*" + usbrelay_port + "' > /tmp/cron.backup");
...
function GetWifiUpdate1(force_wifi) {
	...
	if (force_wifi != null) {
		...
		setControlsEnabled(false, true, "Taking relay " + force_wifi);
	}
