...
<fieldset id="nowebcam">
	...
	<em><span class="nocolumn">Nie wykryto żadnej kamery USB.</span></em>
</fieldset>
...
<fieldset id=webcam>
	...
	<div>
		<label id="webcam_enable_label" class="leftcolumn" for="webcam_enable">Włączony:</label>
		<input id="webcam_enable" class="rightcolumn" type="checkbox" onchange='updateWebcamWanAccess()'/>
	</div>

	<div>
		<label id="webcam_wan_access_label" class="leftcolumn" for="webcam_wan_access">Zdalny dostęp:</label>
		<input id="webcam_wan_access" class="rightcolumn" type="checkbox" />
	</div>

	<div>
		<label id="webcam_device_label" class="leftcolumn" for="webcam_device">Urządzenie:</label>
		<select class='rightcolumn' id='webcam_device' onchange='fillRes(this.value)' >
		</select>
	</div>

	<div>
		<label class='leftcolumn' id='webcam_res_label' for='webcam_res'>Rozdzielczość:</label>
		<select class='rightcolumn' id='webcam_res' >
		</select>
	</div>

	<div>
		<label id="webcam_fps_label" class="leftcolumn" for="webcam_fps">Ilość klatek/s (FPS):</label>
		<input id="webcam_fps" class="rightcolumn" type="text" size='20' maxlength='2' onkeyup='proofreadNumericRange(this,1,59)'/>
	</div>

	<div>
		<label id="webcam_yuv_label" class="leftcolumn" for="webcam_yuv">Obsługa formatu YUYV:</label>
	...
	<div>
		<label id="webcam_username_label" class="leftcolumn" for="webcam_username">Nazwa użytkownika:</label>
		<input id="webcam_username" class="rightcolumn" type="text" size='20'/>
		<em>(opcja)</em>
	</div>
	<div>
		<label id="webcam_password_label" class="leftcolumn" for="webcam_password">Hasło:</label>
...
<fieldset id=webcam_preview>
	<legend class="sectionheader">Podgląd obrazu</legend>
