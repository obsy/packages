...
function saveChanges()
{
	...
	if (errors.length == 0 && uciOriginal.get("firewall", webcamWanAccess, "local_port") != ""+port )
	{
		...
		if(conflict != "")
		{
			errors.push("Nie można włączyć obsługi kamery - konflikt portu z wartością wpisaną w polu " + conflict);
		}
	}
...
function showPreview(enabled, port, username, password)
{
	if(enabled==1)
	{
		...
		setChildText("webcam_info", "Obraz z kamery dostępny jest pod adresem " + webcam_url);
